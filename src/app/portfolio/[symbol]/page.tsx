...
import { ArrowLeft, Activity, Target, ShieldAlert, BarChart3, TrendingUp, TrendingDown, Info, RefreshCcw } from 'lucide-react';
...
export default function DeepTerminal({ params }: { params: Promise<{ symbol: string }> }) {
  ...
  if (deepLoading) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '3rem', color: 'var(--text-muted)' }}>
        <div style={{ width: '22px', height: '22px', border: '2px solid var(--accent)', borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
        <span>Loading deep analytics for <strong style={{ color: 'var(--text-primary)' }}>{symbol}</strong>…</span>
      </div>
    );
  }
  ...
  return (
    <div style={{ padding: '2rem 2.5rem', maxWidth: '1440px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      ...
      {/* Header */}
      <div>
        <Link href="/portfolio" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.4rem', color: 'var(--text-muted)', textDecoration: 'none', fontSize: '0.85rem', marginBottom: '1.25rem' }}>
          <ArrowLeft size={14} /> Back to My Portfolio
        </Link>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
                <div>
                  ...
                  <div style={{ fontSize: '2.2rem', fontWeight: 800, color: (data.technicals?.beta ?? 1) > 1.2 ? 'var(--loss)' : (data.technicals?.beta ?? 1) < 0.8 ? 'var(--profit)' : 'var(--text-primary)', lineHeight: 1 }}>
                    {data.technicals?.beta?.toFixed(2) ?? 'N/A'}
                  </div>
                </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          ...
          <div className="glass-card" style={{ padding: '1.5rem', borderTop: `3px solid ${recColor[rec] || 'var(--accent)'}` }}>
            ...
            {[
              ...
              { label: 'Mean Target', val: `₹${fmt(data.analyst?.target)}`, color: 'var(--text-primary)' },
